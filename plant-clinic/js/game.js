...
PPC.Game = (function () {
  ...
  function askQuestion(q) {
    if (state.phase !== "investigate") return;
    if (state.asked.indexOf(q.id) !== -1) return;
    ...
    if (q.clue) PPC.UI.toast("Answer saved");
  }
  ...
  function aiEstimates(cd, clues) {
    ...
    cd.diseases.forEach(function (d) {
      var gained = 0, max = 0;
      for (var k in d.clues) { max += d.clues[k]; if (clues.indexOf(k) !== -1) gained += d.clues[k]; }
      var pct = max === 0 ? 0 : Math.round(gained / max * 100);
      res.push({ name: d.name, pct: pct });
    });
  ...
  PPC.UI.investigateBar = (function () {
    ...
    var side = "right";
    try {
      if (window.localStorage && window.localStorage.getItem("pixelPlantClinicToolSide") === "left") side = "left";
    } catch (e) { /* Use the default side when storage is unavailable. */ }
    function show(cd, st) {
      PPC.UI.hudClear();
      PPC.UI.hudFor(st);
      if (cur && cur.parentNode) cur.parentNode.removeChild(cur);
      var layer = document.getElementById("ui-layer");
      var bar = PPC.UI.el("div", { id: "investigate-bar", cls: "panel tools-" + side + (expanded ? "" : " collapsed") });
      var header = PPC.UI.el("div", { cls: "tools-header" });
      var toggle = PPC.UI.btn(expanded ? "Close Tools \u25b2" : "Open Tools \u25bc", "tools-toggle", function () {
        expanded = !expanded;
        show(cd, st);
      });
      toggle.setAttribute("aria-expanded", expanded ? "true" : "false");
      toggle.setAttribute("aria-controls", "clinic-tool-actions");
      header.appendChild(toggle);
      var move = PPC.UI.btn(side === "right" ? "\u2190 Move Left" : "Move Right \u2192", "ghost tools-move", function () {
        side = side === "right" ? "left" : "right";
        try {
          if (window.localStorage) window.localStorage.setItem("pixelPlantClinicToolSide", side);
        } catch (e) { /* Side selection remains available for this session. */ }
        show(cd, st);
      });
      move.setAttribute("aria-label", "Move Clinic Tools to the " + (side === "right" ? "left" : "right") + " side");
      header.appendChild(move);
      bar.appendChild(header);
      var row = PPC.UI.el("div", { id: "clinic-tool-actions", cls: "tools-actions" });
      var buttons = [
        [st.zoom ? "\u2190 Zoom Out" : "Inspect Plant", "", function () { PPC.Game.toggleZoom(); }],
        ["Inspection List", "ghost", function () { PPC.Game.openInspectList(); }],
        ["Ask Owner", "", function () { PPC.Game.openInterview(); }],
        ["Patient Chart", "ghost", function () { PPC.Game.openPatientRecord(); }],
        ["Case Notes", "ghost", function () { PPC.Game.openNotebook(); }],
        ["Evidence AI", "ghost", function () { PPC.Game.openComputer(); }],
        ["Diagnose", "warn", function () { PPC.Game.openDiagnose(); }]
      ];
      if (cd.meter) buttons.splice(4, 0, ["Humidity", "ghost", function () { PPC.Game.openMeter(); }]);
      buttons.forEach(function (b) { row.appendChild(PPC.UI.btn(b[0], b[1], b[2])); });
      bar.appendChild(row);
